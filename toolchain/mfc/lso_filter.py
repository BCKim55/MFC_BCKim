"""LSO (Least-Squares Optimized) variable-weight 9-point Gaussian filter design.
...
# Must stay in sync with lso_max_passes on the Fortran side.
...
def compute_lso_params(
    ...
    for tag, d in directions:
        ...
        if sigma_target > 45.0:
            raise ValueError(
                f"LSO filter: sigma = {sigma_target:.1f} cells in {tag} exceeds the " f"stability limit (~45) of the repeated 9-point cascade. Reduce the " f"filter width or coarsen the grid."
            )
        ...
        cons.print(f"  [LSO] {tag}-dir: {n_res_note}sigma_target={sigma_target:.2f} cells, N_passes={n_passes}, L2_err={err:.2e}, peak_partial_gain={gain:.1e}")
...
def lso_namelist_lines(lso_params: dict, prefix: str = "lso_") -> str:
    ...
    lso_a_x). ``prefix`` sets the namelist variable prefix in the emitted lines:
    "lso_pp_" for the post_process filter."""
    lines = []
