...
DESCRIPTIONS = {
    ...
    "lso_pp2_n_passes_x": "Number of stage-2 post-process filter passes in x (derived; > 0 when a wide target is split across two cascades)",
    "lso_pp2_n_passes_y": "Number of stage-2 post-process filter passes in y (derived; > 0 when a wide target is split across two cascades)",
    "lso_pp2_n_passes_z": "Number of stage-2 post-process filter passes in z (derived; > 0 when a wide target is split across two cascades)",
    "lso_pp2_a_x": "Stage-2 post-process per-pass stencil coefficients in x (5 x lso_max_passes, column-major)",
    "lso_pp2_a_y": "Stage-2 post-process per-pass stencil coefficients in y (5 x lso_max_passes, column-major)",
    "lso_pp2_a_z": "Stage-2 post-process per-pass stencil coefficients in z (5 x lso_max_passes, column-major)",
