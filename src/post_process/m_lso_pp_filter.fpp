...
!! (toolchain/mfc/lso_filter.py) via post_process.inp. Wide targets are split across two
!! cascades (lso_pp_a_* then lso_pp2_a_*) whose variances add to the requested width, since
!! a single cascade is only well conditioned up to ~40 cells.
!!
...
contains
    ...
    !> Apply the LSO filter to each field of q_cons_vf in place: the stage-1 pass set, followed by the stage-2 set when the
    !! toolchain split a wide target across two cascades (a single cascade is only well conditioned up to ~40 cells).
